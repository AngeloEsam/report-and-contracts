...
import { Contract7RequestDto } from '../types/contract.types';
...
const TEMPLATE_BASE_PATH = './src/utilities/contract-and-report/templates/contract 7';

export class Contract7Service {

    async generateContract(data: Contract7RequestDto): Promise<Buffer> {
        ...
        if (data.repeated.safetyEquipmentRows && data.repeated.safetyEquipmentRows.length > 0) {
            repeatedReplacements.push({
                repeatedTempletePath: `${TEMPLATE_BASE_PATH}/safetyEquipmentRow.html`,
                placeholder: 'safetyEquipmentRows',
                changeable: data.repeated.safetyEquipmentRows
            });
        }
...
export default new Contract7Service();
