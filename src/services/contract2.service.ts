...
import { Contract2RequestDto } from '../types/contract.types';
...
const TEMPLATE_BASE_PATH = './src/utilities/contract-and-report/templates/contract 2';

export class Contract2Service {

    async generateContract(data: Contract2RequestDto): Promise<Buffer> {
        ...
        if (data.repeated.refillingPrices && data.repeated.refillingPrices.length > 0) {
            repeatedReplacements.push({
                repeatedTempletePath: `${TEMPLATE_BASE_PATH}/refillingPrices.html`,
                placeholder: 'refillingPrices',
                changeable: data.repeated.refillingPrices
            });
        }
...
export default new Contract2Service();
