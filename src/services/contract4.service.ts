...
import { Contract4RequestDto } from '../types/contract.types';
...
const TEMPLATE_BASE_PATH = './src/utilities/contract-and-report/templates/contract 4';

export class Contract4Service {

    async generateContract(data: Contract4RequestDto): Promise<Buffer> {
        ...
        if (data.repeated.costRows && data.repeated.costRows.length > 0) {
            repeatedReplacements.push({
                repeatedTempletePath: `${TEMPLATE_BASE_PATH}/costRow.html`,
                placeholder: 'costRows',
                changeable: data.repeated.costRows
            });
        }
...
export default new Contract4Service();
